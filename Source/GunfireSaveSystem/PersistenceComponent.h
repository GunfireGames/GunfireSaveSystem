...
public:
	...
	bool HasValidPersistentId() const { return UniqueId.IsValid() || !SaveKey.IsNone(); }
...
protected:
	bool CanPersistObject(AActor* pObject) const;
...
protected:
	...
	// The name of the save container to store save data for this actor in. If this is not set a value will be
	// automatically generated based on the level name. Typically this should not be set, it's for special cases such
	// as the player character, which need to be persisted but aren't placed in a specific level.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, AdvancedDisplay, Category = Persistence)
	FName SaveKey;

	// True if this actor has any saved properties set to different values from the defaults (ie, they are editable on
	// instances and they were changed on this instance).
	UPROPERTY(VisibleInstanceOnly, Category = Persistence)
	bool HasModifiedSaveValues = false;

	// Adding override for actors who normally need to persist, but need to avoid persistence at certain key times. 
	...
	// Public so they can be set in constructors, but these shouldn't be changed at runtime otherwise.
public:
	...
	// If true, this object will persist when it's destroyed, and on a subsequent load of the map the object will be
	// removed.
	UPROPERTY(EditDefaultsOnly, Category = Persistence)
	bool PersistDestroyed = false;

	// Destroys the owner of this component safely. Called from the persistence container for persistent actors that
	// should persist being destroyed.
